...
public interface UserPortfolioSkillsCommunicationRepository extends JpaRepository<UserPortfolioSkillsCommunication, Long> {
    ...
    @Query("SELECT p FROM UserPortfolioSkillsCommunication p WHERE p.userInfo = :user_id")
    List<UserPortfolioSkillsCommunication> findByUserId(@Param("user_id") UserInfo user);
}
